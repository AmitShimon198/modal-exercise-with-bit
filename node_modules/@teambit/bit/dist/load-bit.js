...
function attachVersionsFromBitmap(config, consumerInfo) {
  ...
  try {
    ...
    _bitMap().default.removeNonComponentFields(parsedBitMap); // Do nothing here, invalid bitmaps will be handled later
  ...
  const result = Object.entries(rawConfig).reduce((acc, [aspectId, aspectConfig]) => {
    let newAspectEntry = aspectId; // In case the id already has a version we don't want to get it from the bitmap
...
async function requireAspects(aspect, runtime) {
  ...
  if (!runtimeFile) return; // eslint-disable-next-line
...
function shouldLoadInSafeMode() {
  ...
  const safeModeCommands = ['cc', 'clear-cache', 'init', 'cat-scope', 'cat-object', 'cat-component', 'cmp', 'cat-lane', 'login', 'logout', 'config'];
...
function clearGlobalsIfNeeded() {
  ...
  _packageJsonTransformer().PackageJsonTransformer.packageJsonTransformersRegistry = []; // @ts-ignore

  _dependencyResolver().DependencyResolver.getWorkspacePolicy = undefined; // @ts-ignore

  _manyComponentsWriter().default.externalInstaller = {};
  _config2().ExtensionDataList.coreExtensionsNames = new Map(); // @ts-ignore

  _workspaceConfig().default.workspaceConfigEnsuringRegistry = undefined; // @ts-ignore

  _workspaceConfig().default.workspaceConfigIsExistRegistry = undefined; // @ts-ignore
