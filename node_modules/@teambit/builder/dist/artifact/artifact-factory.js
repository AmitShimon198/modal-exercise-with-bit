...
exports.ArtifactFactory = exports.DEFAULT_CONTEXT = void 0;
...
class ArtifactFactory {
  constructor(storageResolverSlot) {
    this.storageResolverSlot = storageResolverSlot;
  }

  getResolver(resolvers, name) {
  ...
  createFromComponent(context, component, def, task) {
    ...
    return new (_artifact().Artifact)(def, storageResolver, new (_artifactFiles().ArtifactFiles)(paths), rootDir, task);
  }
  ...
  toComponentMap(context, artifactMap) {
    return _component().ComponentMap.as(context.components, component => {
      ...
      return new (_artifactList().ArtifactList)(artifacts);
    });
  }
  ...
  generate(context, defs, task) {
    ...
    defs.forEach(def => {
      ...
      if (artifactContext === 'env') {
        ...
        if (paths && paths.length) {
          const artifact = new (_artifact().Artifact)(def, this.getStorageResolver(def), new (_artifactFiles().ArtifactFiles)(this.resolvePaths(rootDir, def)), rootDir, task);
