...
function calculatePipelineOrder(taskSlot, envs, pipeNameOnEnv = 'getBuildPipe', tasks = [], skipTests = false) {
  ...
  envs.forEach(envDefinition => {
    if (envDefinition.env.getPipe) {
      ...
      throw new Error(`Fatal: a breaking API has introduced. Please change "getPipe()" method on "${envDefinition.id}" to "getBuildPipe()"`);
    }
...
function addDependenciesToGraph(graphs, pipeline, task) {
  ...
  task.dependencies.forEach(dependency => {
    const {
      aspectId,
      name
    } = _buildTask().BuildTaskHelper.deserializeId(dependency);
