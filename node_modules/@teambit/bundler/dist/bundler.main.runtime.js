...
class BundlerMain {
  ...
  async devServer(components) {
    const envRuntime = await this.envs.createEnvironment(components); // TODO: this must be refactored away from here. this logic should be in the Preview.
  ...
  static async provider([pubsub, envs, graphql], config, [runtimeSlot]) {
    const bundler = new BundlerMain(config, pubsub, envs, new (_devServer2().DevServerService)(pubsub, runtimeSlot), runtimeSlot);
