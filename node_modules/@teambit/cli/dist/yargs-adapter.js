...
class YargsAdapter {
  constructor(commanderCommand) {
    ...
    this.describe = _chalk().default.yellow(commanderCommand.description);
  ...
  handler(argv) {
    ...
    const argsValues = enteredArgs.map(a => argv[a]); // a workaround to get a flag syntax such as "--all [version]" work with yargs.

    const flags = Object.keys(argv).reduce((acc, current) => {
      if (current === '_' || current === '$0' || current === '--') return acc; // const flagName = current.split(' ')[0];
