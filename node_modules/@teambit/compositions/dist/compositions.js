...
function _baseUiRouting() {
  const data = require("@teambit/base-ui.routing.native-link");
...
function Compositions({
  ...
  const properties = (0, _docsUiQueries().useDocs)(component.id); // reset selected composition when component changes.
  ...
  const currentCompositionUrl = (0, _previewUi().toPreviewUrl)(component, 'compositions', selected === null || selected === void 0 ? void 0 : selected.identifier);
  const [compositionParams, setCompositionParams] = (0, _react().useState)({});
  const queryParams = (0, _react().useMemo)(() => _queryString().default.stringify(compositionParams), [compositionParams]); // collapse sidebar when empty, reopen when not
  ...
  }, /*#__PURE__*/_react().default.createElement(_baseUiRouting().NativeLink, {
    external: true,
    href: currentCompositionUrl,
