...
class DependencyLinker {
  ...
  async link(rootDir, rootPolicy, componentDirectoryMap, options = {}) {
    ...
    if (linkingOpts.legacyLink) {
      ...
      const legacyStringIds = componentDirectoryMap.toArray().map(([component]) => component.id._legacy.toString()); // @todo, Gilad, it's better not to use the legacyLink here. it runs the consumer onDestroy
      ...
      const legacyResults = await (0, _consumer().link)(legacyStringIds, (_linkingOpts$rewire = linkingOpts.rewire) !== null && _linkingOpts$rewire !== void 0 ? _linkingOpts$rewire : false, false);
    ...
    if (linkingOpts.linkNestedDepsInNM) {
      result.nestedDepsInNmLinks = await this.addSymlinkFromComponentDirNMToWorkspaceDirNM(finalRootDir, componentDirectoryMap);
    ...
    componentDirectoryMap.map((_dir, comp) => {
      componentIdsWithoutVersions.push(comp.id.toString({
        ignoreVersion: true
      }));
      return undefined;
    });
  ...
  addSymlinkFromComponentDirNMToWorkspaceDirNM(rootDir, componentDirectoryMap) {
    ...
    const getPackagesFoldersToLink = (dir, parent) => {
      const folders = _fsExtra().default.readdirSync(dir, {
        ...
        const moduleName = parent ? `${parent}/${dirent.name}` : dirent.name; // This is a scoped package, need to go inside
    ...
    const linksOfAllComponents = componentDirectoryMap.toArray().map(([component, dir]) => {
      ...
      if (!_fsExtra().default.existsSync(compDirNM)) return undefined; // TODO: support modules with scoped packages (start with @) - we need to make this logic 2 levels

      const componentPackageName = (0, _componentIdToPackageName().default)(component.state._consumer);

      const innerNMofComponentInNM = _path().default.join(rootNodeModules, componentPackageName); // If the folder itself is a symlink, do not try to symlink inside it
      ...
      const oneComponentLinks = packagesFoldersToLink.map(folderEntry => {
        ...
        const linkSrc = folderEntry.path; // This works as well, consider using it instead
  ...
  async linkDepsResolvedFromEnv(componentDirectoryMap) {
    ...
    const componentsNeedLinksP = componentDirectoryMap.toArray().map(async ([component, dir]) => {
      ...
      const resolvedFromEnv = policy.getResolvedFromEnv(); // Nothing should be resolved from env, do nothing
    ...
    await Promise.all(componentsNeedLinksP); // Stop if there are not components needs to be linked
  ...
  linkCoreAspect(dir, id, name, packageName, hasLocalInstallation = false) {
    ...
    try {
      ...
      const aspectPath = _path().default.resolve(_path().default.join(module.path, '..', '..')); // in this case we want the symlinks to be relative links
  ...
  removeSymlinkTarget(targetPath, hasLocalInstallation = false) {
    ...
    try {
      targetStat = _fsExtra().default.lstatSync(targetPath); // eslint-disable-next-line no-empty
    } catch (e) {}
  ...
  linkNonAspectCorePackages(rootDir, name, packageName = `@teambit/${name}`, skipExisting = false) {
    ...
    try {
      ...
      const resolvedPath = _path().default.resolve(_path().default.join(module.path, '..', '..')); // in this case we want the symlinks to be relative links
      // Using the fs module to make sure it is relative to the target
  ...
  linkHarmony(dirMap, rootDir) {
    const name = 'harmony';
    return this.linkNonAspectCorePackages(rootDir, name);
  }

  linkTeambitLegacy(dirMap, rootDir) {
    const name = 'legacy';
    return this.linkNonAspectCorePackages(rootDir, name);
  }
