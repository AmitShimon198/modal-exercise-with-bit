...
exports.DependencyResolverMain = exports.NPM_REGISTRY = exports.BIT_DEV_REGISTRY = void 0;
...
class DependencyResolverMain {
  ...
  async extractDepsFromLegacy(component, policy) {
    ...
    dependencyList.forEach(dep => {
      const found = componentPolicy.find(dep.id); // if no policy found, the dependency was auto-resolved from the source code
  ...
  async getWorkspaceManifest(name = _constants2().ROOT_NAME, version = new (_semver().SemVer)('1.0.0'), rootPolicy, rootDir, components, options = defaultCreateFromComponentsOptions) {
    ...
    const workspaceManifestFactory = new (_manifest().WorkspaceManifestFactory)(this);
  ...
  getInstaller(options = {}) {
    ...
    const postInstallSubscribers = this.getPostInstallSubscribers(); // TODO: we should somehow pass the cache root dir to the package manager constructor

    return new (_dependencyInstaller().DependencyInstaller)(packageManager, this.aspectLoader, this.logger, options.rootDir, cacheRootDir, preInstallSubscribers, postInstallSubscribers);
  }
  ...
  getLinker(options = {}) {
    const linkingOptions = Object.assign({}, defaultLinkingOptions, (options === null || options === void 0 ? void 0 : options.linkingOptions) || {}); // TODO: we should somehow pass the cache root dir to the package manager constructor
  ...
  getVersionResolver(options = {}) {
    ...
    return new (_dependencyVersionResolver().DependencyVersionResolver)(packageManager, cacheRootDir);
  }
  /**
  ...
  async getProxyConfig() {
    ...
    let httpsProxy = proxyConfigFromDepResolverConfig.httpsProxy; // Take config from the aspect config if defined
  ...
  getProxyConfigFromDepResolverConfig() {
    ...
    return {
      ...
      strictSSL: ((_this$config$strictSs = this.config.strictSsl) === null || _this$config$strictSs === void 0 ? void 0 : _this$config$strictSs.toLowerCase()) === 'true'
    };
  }
  ...
  async getProxyConfigFromGlobalConfig() {
  ...
  async getRegistries() {
    ...
    const bitDefaultRegistry = getDefaultBitRegistry(); // Override default registry to use bit registry in case npmjs is the default - bit registry will proxy it
    // We check also NPM_REGISTRY.startsWith because the uri might not have the trailing / we have in NPM_REGISTRY

    if (this.config.installFromBitDevRegistry && (!registries.defaultRegistry.uri || registries.defaultRegistry.uri === NPM_REGISTRY || NPM_REGISTRY.startsWith(registries.defaultRegistry.uri))) {
  ...
  getBitAuthConfig(bitScopeRegistry) {
    ...
    let bitOriginalAuthValue = bitScopeRegistry === null || bitScopeRegistry === void 0 ? void 0 : bitScopeRegistry.originalAuthValue; // In case there is no auth configuration in the npmrc, but there is token in bit config, take it from the config
  ...
  async mergeVariantPolicies(configuredExtensions) {
    ...
    let policiesFromEnv = variantPolicyFactory.getEmpty();
  ...
  async resolveRequireableExtensionManifestDepsVersionsRecursively( // Allow getting here string for lazy load the component
  ...
  parentComponent, manifest) {
    ...
    const updateDirectDepsVersions = deps => {
      return (0, _pMapSeries().default)(deps, async dep => {
        ...
        if (!dep.id) return; // In case of core aspect, do not update the version, as it's loaded to harmony without version

        if (this.aspectLoader.isCoreAspect(dep.id)) return; // Lazily get the parent component
  ...
  static async provider([envs, loggerExt, configMain, aspectLoader, componentAspect, graphql, globalConfig], config, [rootPolicyRegistry, policiesRegistry, packageManagerSlot, dependencyFactorySlot, preInstallSlot, postInstallSlot]) {
    ...
    componentAspect.registerShowFragments([new (_showFragments().DependenciesFragment)(dependencyResolver), new (_showFragments().DevDependenciesFragment)(dependencyResolver), new (_showFragments().PeerDependenciesFragment)(dependencyResolver)]); // TODO: solve this generics issue and remove the ts-ignore
    // @ts-ignore
    ...
    (0, _tagModelComponent().registerUpdateDependenciesOnTag)(dependencyResolver.updateDepsOnLegacyTag.bind(dependencyResolver));
    (0, _exportScopeComponents().registerUpdateDependenciesOnExport)(dependencyResolver.updateDepsOnLegacyExport.bind(dependencyResolver));
