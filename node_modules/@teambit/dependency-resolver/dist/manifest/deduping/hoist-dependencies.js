...
function hoistDependencies(depIdIndex) {
  const result = getEmptyDedupedDependencies(); // TODO: handle git urls
...
function handlePreserved(dedupedDependencies, packageName, indexItem) {
  const preservedVersion = indexItem.metadata.preservedVersion; // Not preserved, move on
...
function addOneOccurrenceToRoot(dedupedDependencies, packageName, indexItems) {
  ...
  const indexItem = indexItems[0]; // if (indexItem.lifecycleType !== PEER_DEP_LIFECYCLE_TYPE) {
...
function handlePeersOnly(dedupedDependencies, packageName, indexItems) {
  ...
  try {
    (0, _semverIntersect().intersect)(...allRanges); // Add to peers for each component to make sure we are getting warning from the package manager about missing peers

    indexItems.map(addToComponentDependenciesMapInDeduped(dedupedDependencies, packageName));
  } catch (e) {
    var _dedupedDependencies$;

    indexItems.map(addToComponentDependenciesMapInDeduped(dedupedDependencies, packageName)); // There are peer version with conflicts, let the user know about it
...
function handleExactVersionsOnly(dedupedDependencies, packageName, indexItems) {
  const allVersions = indexItems.map(item => item.range); // Add most common version to root
...
function handleRangesAndVersions(dedupedDependencies, packageName, indexItems, groups) {
  ...
  const mostCommonVersion = findMostCommonVersion(allVersions); // Include versions here since we might have a specific version which match the best version as well
...
function findBestRange(ranges) {
  ...
  let i = 0; // Since it's already sorted by count, once we found match we can stop looping

  while (result.count === 0 && i < sortedByTotal.length) {
    const combinationWithTotal = sortedByTotal[i];

    try {
      const intersectedRange = (0, _semverIntersect().intersect)(...combinationWithTotal.combination);
      result.intersectedRange = intersectedRange;
      result.ranges = combinationWithTotal.combination;
      result.count = combinationWithTotal.total; // eslint-disable-next-line
    } catch (e) {}
...
function addToComponentDependenciesMapInDeduped(dedupedDependencies, packageName, filterFunc) {
  return indexItem => {
    ...
    compEntry[depKeyName] = Object.assign({}, compEntry[depKeyName], {
...
function groupByRangeOrVersion(indexItems) {
  ...
  indexItems.forEach(item => {
    const parsed = (0, _semverIntersect().parseRange)(_semver().default.validRange(item.range));
...
function arrayCombinations(array) {
  ...
  const all = []; // eslint-disable-next-line
...
function intersectNoThrow(...args) {
  try {
    return (0, _semverIntersect().intersect)(...args);
  } catch (e) {
    return undefined;
  }
}
