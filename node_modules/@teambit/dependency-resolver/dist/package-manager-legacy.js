  ...
  async capsulesInstall(capsules, opts = {}) {
    ...
    if (packageManager === 'npm' || packageManager === 'yarn' || packageManager === 'pnpm') {
      ...
      await (0, _pMapSeries().default)(capsules, async capsule => {
        ...
        longProcessLogger.logProgress(componentId); // TODO: remove this hack once harmony supports ownExtensionName
        ...
        installProc.stderr.on('data', d => this.logger.warn(`${componentId}, ${d.toString()}`)); // eslint-disable-next-line @typescript-eslint/no-floating-promises
  ...
  async runInstallInFolder(folder, opts = {}) {
    ...
    if (packageManager === 'npm') {
      ...
      await new Promise((resolve, reject) => {
        // @ts-ignore
        child.stdout.on('data', d => this.logger.info(`${folder} ${d.toString()}`)); // @ts-ignore

        child.stderr.on('data', d => this.logger.warn(`${folder} ${d.toString()}`)); // eslint-disable-next-line @typescript-eslint/no-floating-promises
...
function linkBitLegacyInCapsule(capsule) {
  ...
  const localBitLegacyPath = getLocalBitLegacyPath(); // if there are no deps, sometimes the node_modules folder is not created
