...
class EnvsMain {
  ...
  createRuntime(components) {
    return new (_runtime().Runtime)(this.aggregateByDefs(components), this.logger);
  ...
  aggregateByDefs(components) {
    ...
    return Object.keys(envsMap).map(key => {
      return new (_runtime().EnvRuntime)(key, envsMap[key].env, envsMap[key].components);
    });
  }
  ...
  static async provider([graphql, loggerAspect, component, cli], config, [envSlot, serviceSlot], context) {
    ...
    cli.register(new (_envs().EnvsCmd)(envs, component));
