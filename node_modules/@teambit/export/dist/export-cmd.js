...
${(0, _constants().WILDCARD_HELP)('export remote-scope')}`);
  ...
  async report([remote, ids = []], {
    ...
    const lanesOutput = () => {
      ...
      return _chalk().default.green(`exported the following ${exportedLanes.length} lane(s):
${exportedLanes.map(l => `${_chalk().default.bold(l.name)} (${l.components.length} components)`).join('\n')}\n\n`);
    };
