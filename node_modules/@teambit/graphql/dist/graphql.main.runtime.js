...
exports.GraphqlMain = exports.Verb = void 0;
...
class GraphqlMain {
  ...
  async createServer(options) {
    ...
    app.use( // @ts-ignore todo: it's not clear what's the issue.
    (0, _cors().default)({
  ...
  getModuleDependencies(extensionId) {
    ...
    const ids = deps.map(dep => dep.id); // @ts-ignore check :TODO why types are breaking here.
  ...
  static async provider([loggerFactory], config, [moduleSlot, graphQLServerSlot], context) {
    const logger = loggerFactory.createLogger(_graphql2().GraphqlAspect.id);
    return new GraphqlMain(config, moduleSlot, context, new (_graphqlSubscriptions().PubSub)(), logger, graphQLServerSlot);
  }

}

exports.GraphqlMain = GraphqlMain;
(0, _defineProperty2().default)(GraphqlMain, "slots", [_harmony().Slot.withType(), _harmony().Slot.withType()]);
