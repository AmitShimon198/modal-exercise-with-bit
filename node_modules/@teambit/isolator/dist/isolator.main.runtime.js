...
class IsolatorMain {
  ...
  constructor(dependencyResolver, logger, componentAspect, graphBuilder) {
  ...
  async isolateComponents(seeders, opts = {}, legacyScope) {
    ...
    return new (_network().Network)(capsuleList, seeders, this.getCapsulesRootDir(opts.baseDir));
  }
  ...
  async createCapsules(components, opts, legacyScope) {
    ...
    const capsulesDir = this.getCapsulesRootDir(opts.baseDir);
  ...
  getCapsulesRootDir(baseDir) {
    return _path().default.join(CAPSULES_BASE_DIR, (0, _objectHash().default)(baseDir));
  }
...
(0, _defineProperty2().default)(IsolatorMain, "dependencies", [_dependencyResolver().DependencyResolverAspect, _logger2().LoggerAspect, _component().ComponentAspect, _graph().GraphAspect]);
