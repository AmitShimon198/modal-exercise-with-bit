...
class JestTester {
  ...
  buildTestsObj(aggregatedResult, components, testerContext, config) {
    const testsSuiteResult = components.toArray().map(([component, testsFiles]) => {
      ...
      if ((testsFiles === null || testsFiles === void 0 ? void 0 : testsFiles.length) === 0) return;
      const tests = testsFiles.map(test => {
        var _test$testExecError;
        ...
        const error = {
          ...
          error: (_test$testExecError = test.testExecError) === null || _test$testExecError === void 0 ? void 0 : _test$testExecError.message
        };
