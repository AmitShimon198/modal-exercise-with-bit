...
function _mergeLanes() {
  const data = require("@teambit/legacy/dist/consumer/lanes/merge-lanes");
...
class LanesMain {
  ...
  static async provider([cli, scope, workspace, graphql]) {
    ...
    if (!isLegacy) {
      ...
      laneCmd.commands = [new (_lane().LaneListCmd)(lanesMain, workspace, scope), new (_lane().LaneShowCmd)(lanesMain, workspace, scope), new (_lane().LaneCreateCmd)(lanesMain), new (_lane().LaneMergeCmd)(lanesMain), new (_lane().LaneRemoveCmd)(lanesMain), new (_lane().LaneTrackCmd)(lanesMain), new (_lanesModules().LaneDiffCmd)(workspace, scope)];
...
(0, _defineProperty2().default)(LanesMain, "dependencies", [_cli().CLIAspect, _scope().ScopeAspect, _workspace().WorkspaceAspect, _graphql().GraphqlAspect]);
