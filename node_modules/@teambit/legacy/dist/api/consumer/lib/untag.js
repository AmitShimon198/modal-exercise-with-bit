...
async function unTagAction(version, force, soft, id) {
  ...
  const untag = async () => {
    if (idHasWildcard) {
      return (0, _untagComponent().removeLocalVersionsForComponentsMatchedByWildcard)(consumer, version, force, id);
    }
    ...
    if (id) {
      ...
      const result = await (0, _untagComponent().removeLocalVersion)(consumer.scope, bitId, version, force);
      return [result];
    } // untag all


    return (0, _untagComponent().removeLocalVersionsForAllComponents)(consumer, version, force);
  };
