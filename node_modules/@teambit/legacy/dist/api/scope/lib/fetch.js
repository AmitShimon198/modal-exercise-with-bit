...
async function fetch(path, ids, // ids type are determined by the fetchOptions.type
fetchOptions, headers) {
  ...
  const args = _objectSpread({
    path,
    ids
  }, fetchOptions); // This might be undefined in case of fork process like during bit test command
  ...
  switch (fetchOptions.type) {
    case 'component':
      {
        ...
        const getComponentsWithOptions = async () => {
          ...
          return versionsDependencies.map(versionDep => [{
        ...
        const componentsWithOptions = await getComponentsWithOptions(); // eslint-disable-next-line @typescript-eslint/no-floating-promises
    ...
    case 'lane':
      {
        const laneIds = ids.map(id => _laneId().RemoteLaneId.parse(id));
        const lanes = await scope.listLanes();
        const lanesToFetch = laneIds.map(laneId => {
          const laneToFetch = lanes.find(lane => lane.name === laneId.name); // @todo: throw LaneNotFound, make sure it shows correctly on the client using ssh
    ...
    case 'object':
      {
        const refs = ids.map(id => new (_objects().Ref)(id)); // eslint-disable-next-line @typescript-eslint/no-floating-promises
