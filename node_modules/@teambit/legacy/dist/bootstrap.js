...
require('regenerator-runtime/runtime'); // eslint-disable-next-line @typescript-eslint/no-misused-promises


process.on('unhandledRejection', async err => (0, _handleErrors().handleUnhandledRejection)(err)); // by default Bluebird enables the longStackTraces when env is `development`, or when
...
function verifyNodeVersionCompatibility() {
  ...
  const isCompatible = _semver().default.satisfies(nodeVersion, `>=${MINIMUM_NODE_VERSION}`);

  if (!isCompatible) {
    // eslint-disable-next-line no-console
    console.log(_chalk().default.red(`Node version ${nodeVersion} is not supported, please use Node.js ${MINIMUM_NODE_VERSION} or higher. If you must use legacy versions of Node.js, please use our binary installation methods. https://docs.bit.dev/docs/installation`));
    process.exit(1);
  }
}
...
function enableLoaderIfPossible() {
  const safeCommandsForLoader = ['status', 's', // status alias
  'compile', 'start', 'add', 'show', 'tag', 'build', 'create', 'test', 'install', 'link', 'import', 'log', 'checkout', 'merge', 'diff', 'env', 'envs'];

  if (safeCommandsForLoader.includes(process.argv[2]) && !process.argv.includes('--json') && !process.argv.includes('-j')) {
    _loader().default.on(); // loader.start('loading bit...');
...
function getHarmonyVersion(showValidSemver = false) {
  try {
    const teambitBit = require.resolve('@teambit/bit'); // eslint-disable-next-line


    const packageJson = require(_path().default.join(teambitBit, '../..', 'package.json'));

    if (packageJson.version) return packageJson.version; // this is running locally
