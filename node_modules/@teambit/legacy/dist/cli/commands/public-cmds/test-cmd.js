...
const validForkLevels = (0, _values2().default)(_constants().TESTS_FORK_LEVEL);
let verboseReport = false;

class Test {
  constructor() {
    (0, _defineProperty2().default)(this, "name", 'test [id]');
    (0, _defineProperty2().default)(this, "shortDescription", 'test any set of components with configured tester (component tester or as defined in bit.json)');
  ...
  async action([id], {
    ...
    const testRes = await (0, _consumer().test)(id, forkLevel, all, verbose); // @ts-ignore obsolete code, no need to fix
