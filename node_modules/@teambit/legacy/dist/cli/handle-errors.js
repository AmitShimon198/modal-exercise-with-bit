...
async function handleErrorAndExit(err, commandName, shouldSerialize = false) {
  try {
    ...
    console.error('failed to log the error properly, failure error', e); // eslint-disable-next-line no-console
...
async function logErrAndExit(err, commandName) {
  ...
  await _logger().default.exitAfterFlush(1, commandName);
}
...
function serializeErrAndExit(err, commandName) {
  ...
  const code = err.code && (0, _utils().isNumeric)(err.code) ? err.code : 1; // eslint-disable-next-line @typescript-eslint/no-misused-promises
