...
class ComponentMap {
  ...
  // save component versions per lanes if they're different than the id
  ...
  static fromJson(componentMapObj) {
    const componentMapParams = _objectSpread(_objectSpread({}, componentMapObj), {}, {
      lanes: componentMapObj.lanes ? componentMapObj.lanes.map(lane => ({
        remoteLane: _laneId().RemoteLaneId.parse(lane.remoteLane),
  ...
  toPlainObject(isLegacy) {
    let res = {
      scope: this.scope,
      version: this.version,
      files: isLegacy || !this.rootDir ? this.files.map(file => (0, _utils().sortObject)(file)) : null,
      ...
      lanes: this.lanes.length ? this.lanes.map(l => ({
        remoteLane: l.remoteLane.toString(),
        version: l.version
      })) : null,
  ...
  updatePerLane(currentRemote, currentLaneIds) {
  ...
  async trackDirectoryChangesLegacy(consumer, id) {
  ...
  async trackDirectoryChangesHarmony(consumer, id) {
    ...
    const gitIgnore = (0, _addComponents2().getGitIgnoreHarmony)(consumer.getPath());
    this.files = await (0, _addComponents2().getFilesByDir)(trackDir, consumer.getPath(), gitIgnore);
  }
