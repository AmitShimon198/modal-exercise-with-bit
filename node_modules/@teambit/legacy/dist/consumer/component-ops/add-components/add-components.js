  ...
  async addOrUpdateComponentInBitMap(component) {
    ...
    const componentFilesP = files.map(async file => {
      ...
      if (isImported) {
        ...
        if (!this.id) throw new (_exceptions().MissingComponentIdForImportedComponent)(idFromBitMap.toStringWithoutVersion());
        ...
        if (idOfFileIsDifferent) {
          ...
          throw new (_exceptions().IncorrectIdForImportedComponent)(existingIdWithoutVersion, this.id, file.relativePath);
        } // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!


        if (this._isPackageJsonOnRootDir(file.relativePath, foundComponentFromBitMap)) return null; // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
        ...
        const isGeneratedForUnsupportedFiles = await this._isGeneratedForUnsupportedFiles(file.relativePath, component.componentId, // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
...
you can add the directory these files are located at and it'll change the root dir of the component accordingly`); // we might want to change the behavior here to not throw an error and only change the rootDir to "."
  ...
  _mergeFilesWithExistingComponentMapFiles(componentFiles, existingComponentMapFile) {
    if (this.tests.length) {
      ...
      // won't be overridden
      return (0, _unionWith2().default)((0, _eqBy2().default)((0, _prop2().default)('relativePath')), componentFiles, existingComponentMapFile);
    }
  ...
  async removeExcludedFiles(componentsWithFiles) {
    const files = (0, _flatten2().default)(componentsWithFiles.map(x => x.files.map(i => i.relativePath)));
  ...
  async _mergeTestFilesWithFiles(files) {
    const testFiles = !(0, _isEmpty2().default)(this.tests) ? await this.getFilesAccordingToDsl(files.map(file => file.relativePath), this.tests) : [];
    const resolvedTestFiles = testFiles.map(testFile => {
      if ((0, _utils().isDir)(path().join(this.consumer.getPath(), testFile))) throw new (_exceptions().TestIsDirectory)(testFile);
      return {
  ...
  async addOneComponent(componentPathsStats) {
    ...
    const componentsWithFilesP = Object.keys(componentPathsStats).map(async componentPath => {
      if (componentPathsStats[componentPath].isDir) {
        ...
        if (!filteredMatches.length) throw new (_exceptions().EmptyDirectory)(componentPath);
        ...
        const getTrackDir = () => {
          if (Object.keys(componentPathsStats).length === 1 && this.origin === _constants().COMPONENT_ORIGINS.AUTHORED) {
...
try to avoid excluding files and maybe put them in your .gitignore if it makes sense.`);
    ...
    let componentsWithFiles = await Promise.all(componentsWithFilesP); // remove files that are excluded

    if (!(0, _isEmpty2().default)(this.exclude)) await this.removeExcludedFiles(componentsWithFiles); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
  ...
  getIgnoreList() {
    ...
    const importedComponents = this.bitMap.getAllComponents(_constants().COMPONENT_ORIGINS.IMPORTED);
  ...
  async add() {
    ...
    const missingFiles = (0, _utils().getMissingTestFiles)(this.tests);

    if (!(0, _isEmpty2().default)(missingFiles)) {
      throw new (_exceptions().PathsNotExist)(missingFiles);
    }

    let componentPathsStats = {};
    const resolvedComponentPathsWithoutGitIgnore = (0, _flatten2().default)(await Promise.all(this.componentPaths.map(componentPath => (0, _utils().glob)(componentPath)))); // add excluded list to gitignore to remove excluded files from list

    const resolvedExcludedFiles = await this.getFilesAccordingToDsl(resolvedComponentPathsWithoutGitIgnore, this.exclude);
    this.ignoreList = [...this.ignoreList, ...resolvedExcludedFiles];
    this.gitIgnore = (0, _ignore().default)().add(this.ignoreList); // add ignore list

    const resolvedComponentPathsWithGitIgnore = this.gitIgnore.filter(resolvedComponentPathsWithoutGitIgnore); // Run diff on both arrays to see what was filtered out because of the gitignore file

    const diff = (0, _arrayDifference().default)(resolvedComponentPathsWithGitIgnore, resolvedComponentPathsWithoutGitIgnore);

    if (!(0, _isEmpty2().default)(this.tests) && this.id && (0, _isEmpty2().default)(resolvedComponentPathsWithoutGitIgnore)) {
      const resolvedTestFiles = (0, _flatten2().default)(await Promise.all(this.tests.map(componentPath => (0, _utils().glob)(componentPath))));
  ...
  async addMultipleComponents(componentPathsStats) {
    ...
    const testToRemove = !(0, _isEmpty2().default)(this.tests) ? await this.getFilesAccordingToDsl(Object.keys(componentPathsStats), this.tests) : [];
...
async function getFilesByDir(dir, consumerPath, gitIgnore) {
  ...
  return filteredMatches.map(match => {
    const normalizedPath = (0, _utils().pathNormalizeToLinux)(match); // the path is relative to consumer. remove the rootDir.
...
function getIgnoreListHarmony(consumerPath) {
  const ignoreList = (0, _utils().retrieveIgnoreList)(consumerPath); // the ability to track package.json is deprecated since Harmony
