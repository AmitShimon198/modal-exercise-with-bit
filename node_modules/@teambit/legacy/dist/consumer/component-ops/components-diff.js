...
async function componentsDiff(consumer, ids, version, toVersion, diffOpts) {
  ...
  if (!components) throw new (_showDoctorError().default)('failed loading the components'); // try to resolve ids scope of by components array
  ...
  async function getComponentDiffOfVersion(component) {
    ...
    const versionB = component.id.version; // this function gets called only when version is set

    diffResult.filesDiff = await getFilesDiff(versionFiles, fsFiles, version, versionB); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
  ...
  async function getComponentDiffBetweenVersions(id) {
    ...
    const fromVersionObject = await modelComponent.loadVersion(version, repository); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
    ...
    const toVersionFiles = await toVersionObject.modelFilesToSourceFiles(repository); // $FlowFixMe version and toVersion are set when calling this function
    // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!

    diffResult.filesDiff = await getFilesDiff(fromVersionFiles, toVersionFiles, version, toVersion); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!

    const fromVersionComponent = await modelComponent.toConsumerComponent(version, consumer.scope.name, repository); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
  ...
  async function getComponentDiff(component) {
    ...
    diffResult.filesDiff = await getFilesDiff(modelFiles, fsFiles, component.id.version, component.id.version); // $FlowFixMe we made sure already that component.componentFromModel is defined
...
async function getFilesDiff(filesA, filesB, filesAVersion, filesBVersion, fileNameAttribute = 'relative', color = true) {
  ...
  const filesDiffP = allPaths.map(async relativePath => {
    const getFilePath = async files => {
      ...
      return (0, _saveIntoOsTmp().saveIntoOsTmp)(fileContent);
    };
...
function outputDiffResults(diffResults) {
  return diffResults.map(diffResult => {
    if (diffResult.hasDiff) {
      ...
      const title = _chalk().default.cyan(`${titleSeparator}\n${titleStr}\n${titleSeparator}`); // @ts-ignore since hasDiff is true, filesDiff must be set
