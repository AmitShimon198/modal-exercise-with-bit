...
function componentToPrintableForDiff(component) {
  ...
  const allDevPackages = _objectSpread(_objectSpread(_objectSpread({}, devPackageDependencies), compilerPackageDependencies.devDependencies), testerPackageDependencies.devDependencies);

  const allPackages = _objectSpread(_objectSpread(_objectSpread({}, packageDependencies), compilerPackageDependencies.dependencies), testerPackageDependencies.dependencies);

  const allPeerPackages = _objectSpread(_objectSpread(_objectSpread({}, component.peerPackageDependencies), compilerPackageDependencies.peerDependencies), testerPackageDependencies.peerDependencies);
...
function diffBetweenComponentsObjects(componentLeft, componentRight, {
  ...
  const getDepDiffType = (left, right) => {
    ...
    } catch (err) {// the semver is probably a range, no need to compare, just fallback to the "changed"
    }
...
function getExtensionsConfigOutput(componentLeft, componentRight) {
  ...
  const rightExtensionsIds = Object.keys(rightExtensionsConfigs); // const mutualIds = R.intersection(rightExtensionsIds, rightExtensionsIds);
