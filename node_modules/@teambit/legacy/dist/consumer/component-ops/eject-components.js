...
class EjectComponents {
  ...
  async findNonEjectedDependents() {
    ...
    const notEjectedComponentsDataP = notEjectedData.map(async notEjectedItem => {
      const dependent = await this.consumer.loadComponent(notEjectedItem.id); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
...
please use bit remove command to remove them.`, err);
  ...
  async removeLocalComponents() {
    ...
    await (0, _deleteComponentFiles().default)(this.consumer, this.idsToEject, true);
    await this.consumer.cleanFromBitMap(this.idsToEject, new (_bitId().BitIds)());
  }
