...
class ImportComponents {
  ...
  async importSpecificComponents() {
    ...
    const componentsWithDependencies = await this.consumer.importComponents(bitIds, true, this.options.saveDependenciesAsComponents);
  ...
  async importAccordingToBitMap() {
    ...
    if (componentsIdsToImport.length) {
      ...
      componentsAndDependencies = !this.consumer.isLegacy && this.options.objectsOnly ? await this.consumer.importComponentsObjectsHarmony(componentsIdsToImport, this.options.fromOriginalScope, this.options.allHistory) : await this.consumer.importComponents(_bitId().BitIds.fromArray(idsWithLatestVersion), true);
  ...
  async _getMergeStatus(componentWithDependencies) {
    ...
    const currentComponent = await componentModel.loadVersion( // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
