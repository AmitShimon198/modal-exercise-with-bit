 ...
 */
async function getManipulateDirForExistingComponents(consumer, componentVersion) {
  const id = componentVersion.id;
  const manipulateDirData = [];
  const componentMap = consumer.bitMap.getComponentIfExist(id, {
    ignoreVersion: true
  });
  const version = await componentVersion.getVersion(consumer.scope.objects, false);
...
async function getManipulateDirWhenImportingComponents(bitMap, versionsDependencies, repository) {
  ...
  const manipulateDirDataP = versionsDependencies.map(async versionDependency => {
    ...
    const manipulateDirDependencies = await Promise.all(manipulateDirDependenciesP); // a component might be a dependency and directly imported at the same time, in which case,
  ...
  return (0, _flatten2().default)(manipulateDirData);
}
/**
...
async function getManipulateDirItemFromComponentVersion(componentVersion, bitMap, repository, isDependency) {
