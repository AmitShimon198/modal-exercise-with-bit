...
async function removeComponents({
  ...
  const localResult = !remote ? await removeLocal(consumer, bitIdsLatest, force, track, deleteFiles) : new (_removedLocalObjects().default)();
  return {
    localResult,
    remoteResult
  };
...
async function removeLocal(consumer, bitIds, force, track, deleteFiles) {
  ...
  if (idsToCleanFromWorkspace.length) {
    await (0, _deleteComponentFiles().default)(consumer, idsToCleanFromWorkspace, deleteFiles);
    await (0, _deleteComponentFiles().default)(consumer, removedDependencies, false);
  ...
  return new (_removedLocalObjects().default)(_bitIds().default.uniqFromArray([...idsToCleanFromWorkspace, ...removedComponentIds]), missingComponents, modifiedComponents, removedDependencies, dependentBits, removedFromLane);
}