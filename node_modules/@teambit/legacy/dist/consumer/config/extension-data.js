...
exports.ExtensionDataList = exports.ExtensionDataEntry = exports.REMOVE_EXTENSION_SPECIAL_SIGN = void 0;
...
class ExtensionDataList extends Array {
  ...
  toConfigObject() {
    const res = {};
    this.forEach(entry => {
      if (entry.rawConfig && !(0, _isEmpty2().default)(entry.rawConfig)) {
  ...
  sortById() {
    const arr = (0, _sortBy2().default)((0, _prop2().default)('stringId'), this); // Also sort the config
  ...
  static fromConfigObject(obj) {
    const arr = [];
    (0, _forEachObjIndexed2().default)((config, id) => {
      const isCore = ExtensionDataList.coreExtensionsNames.has(id);
      let entry;

      if (!isCore) {
        const parsedId = _bitId().BitId.parse(id, true);
