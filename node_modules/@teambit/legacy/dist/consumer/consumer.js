...
class Consumer {
  ...
  async loadComponentWithDependenciesFromModel(id, throwIfNotExist = true) {
    ...
    const getModelComponent = async () => {
      ...
      await scopeComponentsImporter.importMany(new (_bitId().BitIds)(id));
  ...
  async importComponents(ids, withAllVersions, saveDependenciesAsComponents) {
    const scopeComponentsImporter = _scopeComponentsImporter().default.getInstance(this.scope);

    const versionDependenciesArr = withAllVersions ? await scopeComponentsImporter.importManyWithAllVersions(ids, false) : await scopeComponentsImporter.importMany(ids);
  ...
  async importComponentsObjectsHarmony(ids, fromOriginalScope = false, allHistory = false) {
    ...
    const versionDependenciesArr = fromOriginalScope ? await scopeComponentsImporter.importManyFromOriginalScopes(ids) : await scopeComponentsImporter.importMany(ids);
  ...
  throwForComponentIssues(components, ignoreIssues) {
    ...
    if (!ignoreIssues) {
      ...
      if (!(0, _isEmpty2().default)(componentsWithBlockingIssues)) throw new (_exceptions2().MissingDependencies)(componentsWithBlockingIssues);
    }
  }
  ...
  async snap({
    ...
    _logger().default.debugAndAddBreadCrumb('consumer.snap', `snapping the following components: {components}`, {
  ...
  async _loadComponentsForTag(ids) {
    const {
      components
    } = await this.loadComponents(ids.toVersionLatest());
    ...
    components.forEach(component => {
      var _component$issues2, _component$issues3;

      const componentMap = component.componentMap;
      if (componentMap.rootDir) return;
      const hasRelativePaths = (_component$issues2 = component.issues) === null || _component$issues2 === void 0 ? void 0 : _component$issues2.getIssue(_componentIssues().IssuesClasses.relativeComponentsAuthored);
      const hasCustomModuleResolutions = (_component$issues3 = component.issues) === null || _component$issues3 === void 0 ? void 0 : _component$issues3.getIssue(_componentIssues().IssuesClasses.MissingCustomModuleResolutionLinks); // leaving this because it can be helpful for users upgrade from legacy
  ...
  async updateComponentsVersions(components) {
    ...
    const isAvailableOnMain = async component => {
  ...
  async cleanFromBitMap(componentsToRemoveFromFs, removedDependencies) {
