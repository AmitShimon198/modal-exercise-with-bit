...
async function saveCheckedOutLaneInfo(consumer, opts) {
  const saveRemoteLaneToBitmap = () => {
    if (opts.remoteLaneScope) {
      ...
      const trackData = consumer.scope.lanes.getRemoteTrackedDataByLocalLane(opts.localLaneName);

      if (!trackData) {
        return; // the lane was never exported
      }

      consumer.bitMap.setRemoteLane(_laneId().RemoteLaneId.from(trackData.remoteLane, trackData.remoteScope));
    }
  };
...
async function getComponentStatus(consumer, id, switchProps) {
  ...
  if (isModified) {
    ...
    const currentComponent = await modelComponent.loadVersion(existingBitMapId.version, // we are here because the head is same as main. so, existingBitMapId.version must be set
