...
async function getComponentStatus(consumer, component, checkoutProps) {
  ...
  const baseComponent = await componentModel.loadVersion(currentlyUsedVersion, consumer.scope.objects);
  ...
  if (version) {
    const currentComponent = await componentModel.loadVersion(newVersion, consumer.scope.objects);
...
mergeResults, checkoutProps) {
  ...
  if (mergeResults) {
    ...
    modifiedStatus = applyModifiedVersion(files, mergeResults, mergeStrategy, componentWithDependencies.component.originallySharedDir);
  }
