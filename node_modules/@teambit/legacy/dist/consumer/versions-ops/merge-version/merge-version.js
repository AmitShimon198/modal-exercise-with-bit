...
async function getComponentStatus(consumer, component, version) {
  ...
  if (unmerged && unmerged.resolved === false) {
    throw new (_generalError().default)(`component ${component.id.toStringWithoutVersion()} has conflicts that need to be resolved first, please use bit merge --resolve/--abort`);
  }
...
async function getMergeStrategyInteractive() {
  try {
    const result = await (0, _prompts().resolveConflictPrompt)(); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
