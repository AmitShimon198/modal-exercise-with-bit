...
function _snapCmd() {
  const data = require("../cli/commands/public-cmds/snap-cmd");
...
class CommandHelper {
  ...
  runCmd(cmd, cwd = this.scopes.localPath, stdio = 'pipe', overrideFeatures) {
  ...
  tagAllWithoutBuild(options = '') {
    const result = this.runCmd(`bit tag -a ${options}`, undefined, undefined, _featureToggle().BUILD_ON_CI);
  ...
  tagScopeWithoutBuild(version = '', options = '') {
    return this.runCmd(`bit tag -s ${version} ${options}`, undefined, undefined, _featureToggle().BUILD_ON_CI);
  }
  ...
  snapAllComponents(options = '', assertSnapped = true) {
    ...
    if (assertSnapped) (0, _chai().expect)(result).to.not.have.string(_snapCmd().NOTHING_TO_SNAP_MSG);
    return result;
  }
  ...
  trackLane(localName, remoteScope, remoteName = '') {
  ...
  getHeadOfLane(laneName, componentName) {
  ...
  exportComponent(id, scope = this.scopes.remote, assert = true, flags = '') {
  ...
  exportAllComponentsAndRewire(scope = this.scopes.remote) {
  ...
  fetchRemoteLane(id) {
    return this.runCmd(`bit fetch ${this.scopes.remote}${_constants().LANE_REMOTE_DELIMITER}${id} --lanes`);
  }
  ...
  mergeRemoteLane(laneName, remoteName = this.scopes.remote, options = '') {
  ...
  create(templateName, componentName, flags = '') {
  ...
  packComponent(id, options, extract = false) {
    ...
    if (extract) {
      if (!options || // We don't just check that it's falsy because usually it's an empty string.
      ...
      const tarballFilePath = resultParsed.metadata.tarPath; // const dir = options.d || options['-out-dir'];
