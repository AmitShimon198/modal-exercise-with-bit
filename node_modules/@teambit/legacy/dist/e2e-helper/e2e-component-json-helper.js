...
class ComponentJsonHelper {
  ...
  addKeyVal(key, val, componentRelativeDir = 'bar') {
    const componentJson = this.read(componentRelativeDir); // Using this to keep the comments
  ...
  composePath(componentRelativeDir = 'bar') {
    return path().join(this.scopes.localPath, componentRelativeDir, _constants().COMPONENT_CONFIG_FILE_NAME);
