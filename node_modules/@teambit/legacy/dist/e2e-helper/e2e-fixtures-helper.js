...
class FixtureHelper {
  constructor(fsHelper, commandHelper, npmHelper, scopes, debugMode, packageJson, scopeHelper) {
  ...
  copyFixtureDir(src, dest) {
    const sourceDir = path().join(this.getFixturesDir(), src);

    _fsExtra().default.copySync(sourceDir, dest);
  }
  ...
  populateComponents(numOfComponents = 3, rewire = true, additionalStr = '', compile = true) {
    const getImp = index => {
...
module.exports = () => 'comp${index}${additionalStr} and ' + ${nextComp}();`;
    ...
    return Array(numOfComponents).fill(null).map((val, key) => `comp${key + 1}${additionalStr}`).join(' and ');
  }
  /**
  ...
  populateExtensions(numOfExtensions = 3) {
    ...
    for (let i = 1; i <= numOfExtensions; i += 1) {
      ...
      this.command.addComponent(`extensions/ext${i}`, {
        m: aspectFileName
      });
    }
  }

  populateComponentsTS(numOfComponents = 3, owner = '@bit', isHarmony = true) {
...
export default () => 'comp${index} and ' + ${nextComp}();`;
  ...
  extractCompressedFixture(filePathRelativeToFixtures, destDir) {
    _fsExtra().default.ensureDirSync(destDir);
