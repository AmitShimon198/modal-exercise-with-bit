...
async function _fetchEnvs(envType) {
  if (envType === 'compiler') {
...
async function _fetchComps(scopeName, namespaces = []) {
  ...
  const listScopeResults = await (0, _consumer().listScope)({
    scopeName,
    showAll: false,
...
var initInteractive = async function initInteractive() {
  ...
  return (0, _consumer().init)(undefined, false, false, false, false, false, answers).then(({
    created,
    addedGitHooks,
    existingGitHooks
  }) => {
...
exports.default = initInteractive;