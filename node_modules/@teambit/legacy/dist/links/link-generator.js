...
function getComponentLinks({
  ...
  const flattenedDependencies = component.flattenedDependencies;
  const links = directDependencies.map(dep => {
    if (!dep.relativePaths || (0, _isEmpty2().default)(dep.relativePaths)) return [];
    ...
    return (0, _flatten2().default)(dependencyLinks);
  });
  ...
  const flattenLinks = (0, _flatten2().default)(links).concat(internalCustomResolvedLinks);
  const {
    postInstallLinks,
    postInstallSymlinks,
    linksToWrite,
  ...
  if (symlinks.length) {
    dataToPersist.addManySymlinks(symlinks.map(symlink => _symlink().default.makeInstance(symlink.source, symlink.dest, undefined, true)));
  } // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
...
function getInternalCustomResolvedLinks(component, componentMap, createNpmLinkFiles) {
  (0, _componentSchema().throwForNonLegacy)(component.isLegacy, getInternalCustomResolvedLinks.name);
  ...
  const customResolvedPathsToProcess = (0, _uniqBy2().default)(JSON.stringify, component.customResolvedPaths).filter(customPath => isResolvePathsInvalid(customPath));
...
function generatePostInstallScript(component, postInstallLinks = [], postInstallSymlinks = []) {
  (0, _componentSchema().throwForNonLegacy)(component.isLegacy, generatePostInstallScript.name); // $FlowFixMe todo: is it possible that writtenPath is empty here?
  // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
...
function addCustomResolveAliasesToPackageJson(component, links) {
  ...
  if ((0, _isEmpty2().default)(resolveAliases)) return false;
...
function getEntryPointForAngularComponent(component, consumer, bitMap) {
  ...
  const filePath = _constants().ANGULAR_BIT_ENTRY_POINT_FILE.find(entryPoint => {
    const fp = path().join(componentRoot, entryPoint);
    return (0, _fsExtra().existsSync)((consumer === null || consumer === void 0 ? void 0 : consumer.toAbsolutePath(fp)) || fp);
  });
