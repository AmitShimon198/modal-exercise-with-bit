...
const _installInOneDirectory = ({
  ...
  const concretePackageManagerProcessOptions = (0, _merge2().default)(defaultPackageManagerProcessOptions, allowedPackageManagerProcessOptions);
  ...
  const defaultArgs = defaultPackageManagerArgs[packageManager] ? defaultPackageManagerArgs[packageManager] : []; // Handle process args

  const concretePackageManagerDefaultArgs = ['install', ...processedModules, ...defaultArgs];
  const concretePackageManagerArgs = rejectNils((0, _concat2().default)(concretePackageManagerDefaultArgs, packageManagerArgs)); // Add npm verbose flag
...
const _getPeerDeps = async dir => {
  ...
  const npmList = await _getNpmList(packageManager, dir); // If the npmList.stdout starts with '{' it's probably a valid json so no throw an error
...
async function getPeerDepsFromNpmList(npmList, packageManager) {
  ...
  const npmListObject = await parseNpmListJsonGracefully(npmList, packageManager); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
