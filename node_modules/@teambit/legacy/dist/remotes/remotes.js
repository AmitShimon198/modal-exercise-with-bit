...
class Remotes extends Map {
  ...
  async fetch(idsGroupedByScope, // option.type determines the id: component-id/lane-id/object-id (hash)
  thisScope, options = {}, context) {
    ...
    _logger().default.debug('[-] Running fetch on remotes, with the following options', fetchOptions); // when importing directly from a remote scope, throw for ScopeNotFound. otherwise, when
...
${failedScopesErr.join('\n')}`);
  ...
  static load(remotes, thisScope) {
    ...
    (0, _utils().forEach)(remotes, (name, host) => {
      const remote = _remote().default.load(name, host, thisScope); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
