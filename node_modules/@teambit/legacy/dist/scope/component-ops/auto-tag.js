...
async function getAutoTagInfo(consumer, changedComponents) {
  ...
  const {
    components
  } = await consumer.loadComponents(idsToLoad);
  ...
  components.forEach(component => {
    ...
    if (!graph.hasNode(idStr)) return; // preorder gets all dependencies and dependencies of dependencies and so on.
...
function potentialComponentsForAutoTagging(consumer, modifiedComponents) {
  const candidateComponentsIds = consumer.bitMap.getAuthoredAndImportedBitIds(); // if a modified component is in candidates array, remove it from the array as it will be already
