...
class ScopeComponentsImporter {
  ...
  async importMany(ids, cache = true, throwForDependencyNotFound = false) {
  ...
  async importWithoutDeps(ids, cache = true) {
  ...
  async importManyWithAllVersions(ids, cache = true, allDepsVersions = false // by default, only dependencies of the latest version are imported
    ...
    const idsWithoutNils = removeNils(ids);
    if ((0, _isEmpty2().default)(idsWithoutNils)) return Promise.resolve([]);
    const versionDependenciesArr = await this.importMany(idsWithoutNils, cache);
    ...
    if (allDepsVersions) {
      ...
      const verDepsOfAllFlattenDeps = await this.importManyWithAllVersions(_bitId().BitIds.uniqFromArray(dependenciesOnly));
  ...
  async fetchWithDeps(ids) {
    _logger().default.debugAndAddBreadCrumb('fetchWithDeps', `ids: {ids}`, {
      ids: ids.toString()
    });

    this.throwIfExternalFound(ids); // avoid race condition of getting multiple "fetch" requests, which later translates into
  ...
  async getExternalMany(ids, remotes, throwForDependencyNotFound = false) {
    if (!ids.length) return [];
    ...
    }, ids, context).fetchFromRemoteAndWrite();
  ...
  async getExternalManyWithoutDeps(ids, remotes, localFetch = false, context = {}) {
    if (!ids.length) return [];
    ...
    await new (_objectsFetcher().ObjectFetcher)(this.repo, this.scope, remotes, {
      withoutDependencies: true
    }, left.map(def => def.id), context).fetchFromRemoteAndWrite();
