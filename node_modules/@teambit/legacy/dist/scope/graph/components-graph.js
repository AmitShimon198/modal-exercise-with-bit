...
async function buildOneGraphForComponents(ids, consumer, loadComponentsFunc, ignoreIds) {
  const graphFromFsBuilder = new (_buildGraphFromFs().GraphFromFsBuilder)(consumer, ignoreIds, loadComponentsFunc);
...
async function buildOneGraphForComponentsUsingScope(ids, scope, direction = 'normal') {
  ...
  await scopeComponentImporter.importMany(_bitId().BitIds.uniqFromArray(allFlattened));
