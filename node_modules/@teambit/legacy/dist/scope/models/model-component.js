...
class Component extends _objects().BitObject {
  ...
  async hasVersion(version, repo, includeOrphaned = true) {
    ...
    const allHashes = await (0, _traverseVersions().getAllVersionHashes)(this, repo, false);
  ...
  get versionsIncludeOrphaned() {
    return _objectSpread(_objectSpread({}, this.orphanedVersions), this.versions);
  }
  ...
  async setDivergeData(repo, throws = true, fromCache = true) {
    if (!this.divergeData || !fromCache) {
      ...
      this.divergeData = await (0, _getDivergeData().getDivergeData)(repo, this, remoteHead, throws);
    }
  }

  getDivergeData() {
    if (!this.divergeData) throw new Error(`getDivergeData() expects divergeData to be populate, please use this.setDivergeData()`);
    return this.divergeData;
  }

  async populateLocalAndRemoteHeads(repo, laneId, lane, remoteLaneId = laneId, remoteScopeName = this.scope) {
    ...
    if (remoteScopeName) {
      ...
      this.laneHeadRemote = await repo.remoteLanes.getRef(_laneId().RemoteLaneId.from(remoteLaneId.name, remoteScopeName), this.toBitId()); // we need also the remote head of main, otherwise, the diverge-data assumes all versions are local

      this.remoteHead = await repo.remoteLanes.getRef(_laneId().RemoteLaneId.from(_constants().DEFAULT_LANE, remoteScopeName), this.toBitId());
    }
  }
  ...
  async latestIncludeRemote(repo) {
    ...
    const allLocalHashes = await (0, _traverseVersions().getAllVersionHashes)(this, repo, false);
  ...
  getVersionToAdd(releaseType = _constants().DEFAULT_BIT_RELEASE_TYPE, exactVersion, incrementBy, preRelease) {
  ...
  addVersion(version, versionToAdd, lane, repo) {
    ...
    if (head && head.toString() !== versionToAdd && // happens with auto-snap
  ...
  version(releaseType = _constants().DEFAULT_BIT_RELEASE_TYPE, incrementBy = 1, preRelease) {
    if (preRelease) releaseType = 'prerelease'; // eslint-disable-next-line @typescript-eslint/no-non-null-assertion

    const increment = ver => semver().inc(ver, releaseType, undefined, preRelease);

    const latest = this.latestVersion();

    if (!latest) {
      return preRelease ? increment(_constants().DEFAULT_BIT_VERSION) : _constants().DEFAULT_BIT_VERSION;
    }
  ...
  toObject() {
    ...
    if (!(0, _lodash().isEmpty)(this.state)) componentObject.state = this.state; // @ts-ignore

    if (!(0, _lodash().isEmpty)(this.orphanedVersions)) componentObject.orphanedVersions = versions(this.orphanedVersions);
    const headStr = this.getHeadStr(); // @ts-ignore
  ...
  async loadVersion(versionStr, repository, throws = true) {
    ...
    if (!version && throws) throw new (_exceptions().VersionNotFound)(versionStr, this.id(), true);
    return version;
  }

  loadVersionSync(version, repository, throws = true) {
    const versionRef = this.getRef(version);
    if (!versionRef) throw new (_exceptions().VersionNotFound)(version, this.id()); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
  ...
  async collectVersionsObjects(repo, versions, ignoreMissingArtifacts) {
    ...
    refsWithoutArtifacts.push(...versionsRefs); // @ts-ignore
    ...
    versionsObjects.forEach(versionObject => {
      ...
      artifactsRefs.push(...(0, _artifactFiles().getRefsFromExtensions)(versionObject.extensions));
    });
    const loadedRefs = [];
...
consider using --ignore-missing-artifacts flag if you're sure the artifacts are in the remote`);
  ...
  toComponentVersion(versionStr) {
    ...
    const versionNum = versionParsed.latest ? this.latest() : versionParsed.resolve(this.listVersionsIncludeOrphaned()); // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
...
make sure to call "getAllIdsAvailableOnLane" and not "getAllBitIdsFromAllLanes"`);
  ...
  async toConsumerComponent(versionStr, scopeName, repository, manipulateDirData) {
    _logger().default.debug(`model-component, converting ${this.id()}, version: ${versionStr} to ConsumerComponent`);
    ...
    const distsP = version.dists ? Promise.all(version.dists.map(loadFileInstance(_sources().Dist))) : null; // @todo: this is weird. why the scopeMeta would be taken from the current scope and not he component scope?
    ...
    const [files, dists, scopeMeta, compiler, tester] = await Promise.all([filesP, distsP, scopeMetaP, compilerP, testerP]);
    const extensions = version.extensions.clone();
    const bindingPrefix = this.bindingPrefix === 'bit' ? '@bit' : this.bindingPrefix; // when generating a new ConsumerComponent out of Version, it is critical to make sure that
    ...
    const consumerComponent = new (_component().default)({
      ...
      compilerPackageDependencies: (0, _clone2().default)(version.compilerPackageDependencies),
      testerPackageDependencies: (0, _clone2().default)(version.testerPackageDependencies),
      // @ts-ignore
      files,
      // @ts-ignore
      dists,
      ...
      log,
      customResolvedPaths: (0, _clone2().default)(version.customResolvedPaths),
  ...
  getLocalVersions() {
    if ((0, _lodash().isEmpty)(this.state) || (0, _lodash().isEmpty)(this.state.versions)) return []; // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
  ...
  async isLocallyChanged(lane, repo) {
    if (lane) {
    ...
    if (!this.divergeData) {
      throw new Error('isLocallyChanged - this.divergeData is missing, please run this.setDivergeData() before calling this method');
    }

    const localVersions = this.getLocalTagsOrHashes();
    if (localVersions.length) return true; // @todo: why this is needed? on main, the localVersion must be populated if changed locally
