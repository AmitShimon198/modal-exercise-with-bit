...
exports.Http = exports.DEFAULT_AUTH_TYPE = exports.Verb = void 0;
...
class Http {
  constructor(graphClient, _token, url, scopeName, proxyConfig, agent, localScopeName) {
  ...
  static async getProxyConfig(checkProxyUriDefined = true) {
    ...
    const httpsProxy = (_obj$CFG_HTTPS_PROXY = obj[_constants().CFG_HTTPS_PROXY]) !== null && _obj$CFG_HTTPS_PROXY !== void 0 ? _obj$CFG_HTTPS_PROXY : obj[_constants().CFG_PROXY]; // If check is true, return the proxy config only case there is actual proxy server defined
    ...
    return {
      ca: obj[_constants().CFG_PROXY_CA],
      cert: obj[_constants().CFG_PROXY_CERT],
      httpProxy,
      httpsProxy,
      key: obj[_constants().CFG_PROXY_KEY],
      noProxy: obj[_constants().CFG_PROXY_NO_PROXY],
      strictSSL: obj[_constants().CFG_PROXY_STRICT_SSL]
    };
  }
  ...
  async throwForNonOkStatus(res) {
    ...
    } catch (e) {// the response is not json, ignore the body.
    }
  ...
  async deprecateMany(ids) {
    const DEPRECATE_COMPONENTS = (0, _graphqlRequest().gql)`
  ...
  async undeprecateMany(ids) {
    const UNDEPRECATE_COMPONENTS = (0, _graphqlRequest().gql)`
      mutation deprecate($bitIds: [String!]!) {
  ...
  getClientVersion() {
    return (0, _bootstrap().getHarmonyVersion)();
  }
