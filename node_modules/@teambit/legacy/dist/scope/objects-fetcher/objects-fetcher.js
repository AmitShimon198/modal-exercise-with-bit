 ...
 */
class ObjectFetcher {
  constructor(repo, scope, remotes, fetchOptions, ids, context = {}) {
    this.repo = repo;
    this.scope = scope;
  ...
  async fetchFromRemoteAndWrite() {
    ...
    const idsGroupedByScope = (0, _scopeComponentsImporter().groupByScopeName)(this.ids);

    _logger().default.debug('[-] Running fetch on remotes, with the following options', this.fetchOptions);
...
${failedScopesErr.join('\n')}`);
    ...
    _logger().default.debug('[-] fetchFromRemoteAndWrite, completed writing all objects');
...
the remote scope "${scopeName}" was not found`);
  ...
  async writeFromSingleRemote(objectsStream, scopeName, objectsQueue, componentsQueue) {
    ...
    const pipelinePromise = (0, _util().promisify)(_stream().pipeline); // add an error listener for the ObjectList to differentiate between errors coming from the
