...
class Repository {
  ...
  async load(ref, throws = false) {
    ...
    try {
      fileContentsRaw = await _fsExtra().default.readFile(this.objectPath(ref));
    } catch (err) {
      if (err.code !== 'ENOENT') {
      ...
      _logger().default.trace(`Failed finding a ref file ${this.objectPath(ref)}.`);
    ...
    const parsedObject = await _object().default.parseObject(fileContents);
  ...
  async list() {
    ...
    return (0, _pMap().default)(refs, ref => this.load(ref), {
      concurrency
    });
  }
  ...
  async _getBitObjectsByHashes(hashes) {
    const bitObjects = await Promise.all(hashes.map(async hash => {
      ...
      if (!bitObject) {
        ...
        await this.scopeIndex.deleteFile(); // @ts-ignore componentId must be set as it was retrieved from indexPath before
  ...
  async loadOptionallyCreateScopeIndex() {
    try {
      ...
      if (err.code === 'ENOENT') {
        const bitObjects = await this.list();
  ...
  async loadRaw(ref) {
    const raw = await _fsExtra().default.readFile(this.objectPath(ref)); // Run hook to transform content pre reading
  ...
  async _writeOne(object) {
    ...
    _logger().default.trace(`repository._writeOne: ${objectPath}`); // Run hook to transform content pre persisting


    const transformedContent = this.onPersist(contents); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
