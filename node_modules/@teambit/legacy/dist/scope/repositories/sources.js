...
to quickly fix the issue, please delete the object at "${this.objects().objectPath(symlink.hash())}"`);
  ...
  put({
    ...
    repo.add(component); // const isObjectShouldBeAdded = (obj) => {
  ...
  removeComponentVersions(component, versions, allVersionsObjects) {
    ...
    const removedRefs = versions.map(version => {
      ...
      if (!versionObject) throw new Error(`removeComponentVersions failed finding a version object of ${refStr}`); // update the snap head if needed

      if (component.getHeadStr() === refStr) {
        if (versionObject.parents.length > 1) {
          throw new Error(`removeComponentVersions found multiple parents for a local (un-exported) version ${version} of ${component.id()}`);
        }
