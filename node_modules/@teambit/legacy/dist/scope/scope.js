...
class Scope {
  ...
  async removeMany(bitIds, force, removeSameOrigin = false, consumer) {
    ...
    const removeComponents = new (_removeModelComponents().default)(this, bitIds, force, removeSameOrigin, consumer);
  ...
  async findDependentBits(bitIds, returnResultsWithVersion = false) {
    const allComponents = await this.list();
    const allComponentVersions = await Promise.all(allComponents.map(async component => {
      const allRefs = await (0, _traverseVersions().getAllVersionHashes)(component, this.objects, false);
      const loadedVersions = await Promise.all(allRefs.map(async ref => {
    ...
    const allScopeComponents = (0, _flatten2().default)(allComponentVersions);
    const dependentBits = {};
    bitIds.forEach(bitId => {
      ...
      if (!(0, _isEmpty2().default)(dependencies)) {
