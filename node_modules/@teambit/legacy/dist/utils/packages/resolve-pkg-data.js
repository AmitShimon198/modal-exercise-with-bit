...
function _packageJson() {
  const data = _interopRequireDefault(require("../../consumer/component/package-json"));
...
function _resolvePkgNameByPath() {
  const data = require("./resolve-pkg-name-by-path");

  _resolvePkgNameByPath = function () {
    return data;
  };
...
function enrichDataFromDependent(packageData, dependentDir) {
  const NODE_MODULES = 'node_modules'; // @todo: currently, the "normalize" makes sure that the package.json is valid, however, due to a
  ...
  const packageFullPath = packageData.fullPath; // The +1 is for the / after the node_modules, we didn't enter it into the NODE_MODULES const because it makes problems on windows
  ...
  const packageName = (0, _resolvePkgNameByPath().resolvePackageNameByPath)(packageRelativePath);
  const packageNameNormalized = packageName.replace('\\', '/');
  const packageVersion = (0, _path2().default)(['dependencies', packageNameNormalized], dependentPackageJson) || (0, _path2().default)(['devDependencies', packageNameNormalized], dependentPackageJson) || (0, _path2().default)(['peerDependencies', packageNameNormalized], dependentPackageJson);
...
function enrichDataFromDependency(packageData) {
  ...
  const packageDir = resolvePackageDirFromFilePath(packageData.fullPath); // don't propagate here since loading a package.json of another folder and taking the version from it will result wrong version
  ...
  const packageInfo = _packageJson().default.loadSync(packageDir); // the version can be empty when creating the package.json for author, or when using custom-module-resolution
  ...
  packageData.packageJsonPath = _path3().default.join(packageDir, _constants().PACKAGE_JSON);
  ...
  if (packageInfo.componentId) {
    ...
    packageData.componentId = new (_bitId().BitId)(packageInfo.componentId);
  }
}
/**
...
function resolvePackageDirFromFilePath(absolutePackageFilePath) {
  ...
  const packageName = (0, _resolvePkgNameByPath().resolvePackageNameByPath)(pathInsideNodeModules);
