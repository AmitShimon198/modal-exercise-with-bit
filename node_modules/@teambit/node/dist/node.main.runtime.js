...
class NodeMain {
  constructor(react, nodeEnv, envs) {
    this.react = react;
  ...
  overrideDependencies(dependencyPolicy) {
    return this.envs.override({
      getDependencies: () => (0, _lodash().merge)(dependencyPolicy, this.nodeEnv.getDependencies())
    });
  }
  ...
  static async provider([envs, react, generator]) {
    const nodeEnv = envs.merge(new (_node2().NodeEnv)(), react.reactEnv);
    envs.registerEnv(nodeEnv);
    generator.registerComponentTemplate([_nodeEnv().nodeEnvTemplate]);
...
(0, _defineProperty2().default)(NodeMain, "dependencies", [_envs().EnvsAspect, _react().ReactAspect, _generator().GeneratorAspect]);
