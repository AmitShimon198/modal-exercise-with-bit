...
class PkgMain {
  static async provider([cli, scope, envs, isolator, logger, workspace, builder, dependencyResolver, componentAspect, graphql], config, [packageJsonPropsRegistry]) {
    ...
    _packageJsonTransformer().PackageJsonTransformer.registerPackageJsonTransformer(pkg.transformPackageJson.bind(pkg)); // TODO: consider passing the pkg instead of packer
  ...
  async getPkgArtifact(component) {
    const artifacts = await this.builder.getArtifactsVinylByExtension(component, _pkg().PkgAspect.id);
  ...
  isPublishedToExternalRegistry(component) {
    ...
    const pkgExt = component.state.aspects.get(_pkg().PkgAspect.id); // By default publish to bit registry
  ...
  getComponentBuildData(component) {
    ...
    if (data) return data; // backward compatibility. the data used to be saved on the pkg aspect rather than on the
    // builder aspect
  ...
  async getSnapManifest(component, snap) {
    var _currentData$pkgJson;

    const idWithCorrectVersion = component.id.changeVersion(snap.hash); // const state = await this.scope.getState(component.id, tag.hash);
    ...
    if (!updatedComponent) {
      throw new (_bitError().BitError)(`snap ${snap.hash} for component ${component.id.toString()} is missing`);
    }

    const currentData = this.getComponentBuildData(updatedComponent); // If for some reason the version has no package.json manifest, return undefined
  ...
  async getPackageTarFile(component) {
    const artifacts = await this.builder.getArtifactsVinylByExtensionAndName(component, _pkg().PkgAspect.id, _packer().TAR_FILE_ARTIFACT_NAME);
