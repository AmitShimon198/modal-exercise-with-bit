...
exports.ReactEnv = exports.AspectEnvType = void 0;
...
const prettierConfig = require('./prettier/prettier.config.js'); // TODO: move to be taken from the key mode of compiler context
...
class ReactEnv {
  ...
  getTsCompiler(mode = 'dev', transformers = [], tsModule = _typescript().default) {
  ...
  writeFileMap(components, local) {
    ...
    const path = (0, _path().join)((0, _os().tmpdir)(), `${Math.random().toString(36).substr(2, 9)}.json`);
  ...
  getDevServer(context, transformers = []) {
    ...
    const componentsDirs = this.getComponentsModulesDirectories(context.components); // const fileMapPath = this.writeFileMap(context.components, true);
    ...
    const componentDevConfig = (0, _webpackConfigComponent2().default)(this.workspace.path, context.id, componentsDirs);
  ...
  async getBundler(context, transformers = []) {
    ...
    const baseProdConfig = (0, _webpackConfigBase().default)(); // const componentProdConfig = componentPreviewProdConfigFactory(fileMapPath);
  ...
  getComponentsModulesDirectories(components) {
    const dirs = components.map(component => {
      return this.pkg.getModulePath(component, {
        absPath: true
      });
    });
    return dirs;
  ...
  getEntriesFromWebpackConfig(config) {
  ...
  getBuildPipe(modifiers = {}) {
    ...
    return [this.getCompilerTask(transformers, (modifiers === null || modifiers === void 0 ? void 0 : (_modifiers$tsModifier2 = modifiers.tsModifier) === null || _modifiers$tsModifier2 === void 0 ? void 0 : _modifiers$tsModifier2.module) || _typescript().default), this.tester.task];
  }

  getCompilerTask(transformers = [], tsModule = _typescript().default) {
