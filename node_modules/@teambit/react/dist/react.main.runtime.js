...
function _react4() {
  const data = require("./react.application");
...
class ReactMain {
   ...
   */
  reactEnv, envs, application, workspace) {
  ...
  registerReactApp(options) {
    if (!this.workspace) return;
    this.application.registerApp(new (_react4().ReactApp)(options.name, options.entry, options.portRange || [3000, 4000], this.reactEnv, this.workspace.path, options.deploy));
  }
  /**
  ...
  overridePackageJsonProps(props) {
    return this.envs.override({
      getPackageJsonProps: () => {
        return _objectSpread(_objectSpread({}, props), this.reactEnv.getPackageJsonProps());
      }
    });
  }
  /**
  ...
  static async provider([envs, jestAspect, tsAspect, compiler, webpack, workspace, graphql, pkg, tester, eslint, prettier, application, generator], config) {
    const reactEnv = new (_react2().ReactEnv)(jestAspect, tsAspect, compiler, webpack, workspace, pkg, tester, config, eslint, prettier);
    const react = new ReactMain(reactEnv, envs, application, workspace);
    graphql.register((0, _react3().reactSchema)(react));
    envs.registerEnv(reactEnv);
    generator.registerComponentTemplate(_react5().componentTemplates);
    generator.registerWorkspaceTemplate(_react5().workspaceTemplates);
    return react;
  }
