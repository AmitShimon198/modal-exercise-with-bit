...
class ScopeComponentLoader {
  ...
  async get(id) {
    ...
    let modelComponent = await this.scope.legacyScope.getModelComponentIfExist(id._legacy); // Search with scope name for bare scopes
    ...
    if (!modelComponent) return undefined; // :TODO move to head snap once we have it merged, for now using `latest`.
  ...
  async getTagMap(modelComponent) {
  ...
  async createStateFromVersion(id, version) {
    ...
    new (_component().Config)(version.mainFile, consumerComponent.extensions), this.scope.componentExtension.createAspectList(consumerComponent.extensions, this.scope.name), _component().ComponentFS.fromVinyls(consumerComponent.files), version.dependencies, consumerComponent);
    return state;
  }
