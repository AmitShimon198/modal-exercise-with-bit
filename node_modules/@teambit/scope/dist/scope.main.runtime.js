...
class ScopeMain {
   ...
   */
  config, tagRegistry, postPutSlot, postDeleteSlot, postExportSlot, postObjectsPersist, preFetchObjects, isolator, aspectLoader, logger) {
  ...
  onTag(tagFn) {
    ...
    const legacyOnTagFunc = async (legacyComponents, options) => {
      ...
      const {
        builderDataMap
      } = await tagFn(components, options);
  ...
  async delete({
    ...
    const componentIds = ids.map(id => _component().ComponentID.fromString(id));
  ...
  async getManifestsGraphRecursively(ids, visited = [], throwOnError = false) {
  ...
  async getResolvedAspects(components, {
    skipIfExists = true
  } = {}) {
  ...
  async requireAspects(components, throwOnError = false) {
  ...
  async import(ids, useCache = true, throwIfNotExist = false) {
    const legacyIds = ids.map(id => {
    ...
    await this.legacyScope.import(_bitId().BitIds.fromArray(withoutOwnScopeAndLocals), useCache);
  ...
  async list(filter, includeCache = false) {
  ...
  async listIds(includeCache = false) {
    const allModelComponents = await this.legacyScope.list();
    const modelComponentsToList = includeCache ? allModelComponents : allModelComponents.filter(modelComponent => this.exists(modelComponent));
    const ids = modelComponentsToList.map(component => _component().ComponentID.fromLegacy(component.toBitIdWithLatestVersion(), component.scope || this.name));
  ...
  async byPattern(patterns, scope = '**') {
    ...
    const targetIds = ids.filter(id => {
      return finalPatterns.some(pattern => (0, _workspaceModules().isMatchNamespacePatternItem)(id.toStringWithoutVersion(), pattern).match);
    });
  ...
  static async provider([componentExt, ui, graphql, cli, isolator, aspectLoader, express, loggerMain], config, [tagSlot, postPutSlot, postDeleteSlot, postExportSlot, postObjectsPersistSlot, preFetchObjectsSlot], harmony) {
    ...
    const scope = new ScopeMain(harmony, legacyScope, componentExt, config, tagSlot, postPutSlot, postDeleteSlot, postExportSlot, postObjectsPersistSlot, preFetchObjectsSlot, isolator, aspectLoader, logger);
...
(0, _defineProperty2().default)(ScopeMain, "slots", [_harmony().Slot.withType(), _harmony().Slot.withType(), _harmony().Slot.withType(), _harmony().Slot.withType(), _harmony().Slot.withType(), _harmony().Slot.withType()]);
(0, _defineProperty2().default)(ScopeMain, "runtime", _cli().MainRuntime);
(0, _defineProperty2().default)(ScopeMain, "dependencies", [_component().ComponentAspect, _ui().UIAspect, _graphql().GraphqlAspect, _cli().CLIAspect, _isolator().IsolatorAspect, _aspectLoader().AspectLoaderAspect, _express().ExpressAspect, _logger().LoggerAspect]);
