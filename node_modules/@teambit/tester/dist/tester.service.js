...
class TesterService {
   ...
   */
  patterns, logger, pubsub, devFiles) {
  ...
  async run(context, options) {
    ...
    if (testCount === 0 && !options.ui) throw new (_exceptions().NoTestFilesFound)(this.patterns.join(','));
    ...
    const patterns = _component().ComponentMap.as(context.components, component => {
      ...
      const componentPatterns = this.devFiles.getDevPatterns(component, _tester().TesterAspect.id);
      return componentPatterns.map(pattern => ({
        path: (0, _path().resolve)(dir, pattern),
        relative: pattern
      }));
    });
