...
class TypescriptCompiler {
  constructor(id, logger, options, tsModule) {
    ...
    (_this$options$tsconfi = this.options.tsconfig).compilerOptions || (_this$options$tsconfi.compilerOptions = {}); // mutate the outDir, otherwise, on capsules, the dists might be written to a different directory and make confusion
  ...
  transpileFile(fileContent, options) {
    ...
    if (result.diagnostics && result.diagnostics.length) {
      ...
      const error = this.tsModule.formatDiagnosticsWithColorAndContext(result.diagnostics, formatHost); // :TODO @david please replace to a more concrete error type and put in 'exceptions' directory here.
  ...
  async postBuild(context) {
    await Promise.all(context.capsuleNetwork.seedersCapsules.map(async capsule => {
      const packageJson = _packageJsonFile().default.loadFromCapsuleSync(capsule.path); // the types['index.ts'] is needed only during the build to avoid errors when tsc finds the
  ...
  async runTscBuild(network) {
    ...
    let nextProject;
    const longProcessLogger = this.logger.createLongProcessLogger('compile typescript components', capsules.length); // eslint-disable-next-line no-cond-assign
