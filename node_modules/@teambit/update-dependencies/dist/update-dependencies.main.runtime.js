...
class UpdateDependenciesMain {
  constructor(scope, logger, builder, dependencyResolver, onPostUpdateDependenciesSlot) {
    this.scope = scope;
    this.logger = logger;
  ...
  async updateDependenciesVersions(depsUpdateItemsRaw, updateDepsOptions) {
    ...
    if (!updateDepsOptions.simulation) {
      await (0, _tagModelComponent().addFlattenedDependenciesToComponents)(this.scope.legacyScope, this.legacyComponents);
    }
    ...
    (0, _tagModelComponent().updateComponentsByTagResult)(this.legacyComponents, legacyBuildResults);
    const publishedPackages = (0, _tagModelComponent().getPublishedPackages)(this.legacyComponents);
...
to bypass this error, use --skip-new-scope-validation flag (not recommended. it could corrupt the components irreversibly)`);
  ...
  async addComponentsToScope() {
    await (0, _pMapSeries().default)(this.legacyComponents, component => this.scope.legacyScope.sources.addSourceFromScope(component));
  }
  ...
  async updateAllDeps() {
    const components = this.depsUpdateItems.map(d => d.component); // current bit ids are needed because we might update multiple components that are depend on
  ...
  static async provider([cli, scope, loggerMain, builder, dependencyResolver], _, [onPostUpdateDependenciesSlot]) {
    const logger = loggerMain.createLogger(_updateDependencies2().UpdateDependenciesAspect.id);
    const updateDependenciesMain = new UpdateDependenciesMain(scope, logger, builder, dependencyResolver, onPostUpdateDependenciesSlot);
...
(0, _defineProperty2().default)(UpdateDependenciesMain, "dependencies", [_cli().CLIAspect, _scope().ScopeAspect, _logger().LoggerAspect, _builder().BuilderAspect, _dependencyResolver().DependencyResolverAspect]);
