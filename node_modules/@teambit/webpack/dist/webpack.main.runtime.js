...
class WebpackMain {
  ...
  createDevServer(context, transformers = []) {
    ...
    const afterMutation = runTransformersWithContext(configMutator.clone(), transformers, transformerContext); // @ts-ignore - fix this
  ...
  createBundler(context, transformers = []) {
    ...
    return new (_webpack4().WebpackBundler)(context.targets, mutatedConfigs, this.logger, _webpack().default);
  }

  createPreviewConfig(targets) {
    return targets.map(target => {
      return (0, _webpack2().configFactory)(target.entries, target.outputPath);
    });
  }
