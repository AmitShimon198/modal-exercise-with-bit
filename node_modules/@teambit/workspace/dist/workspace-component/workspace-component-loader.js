...
class WorkspaceComponentLoader {
  ...
  addMultipleEnvsIssueIfNeeded(component) {
    ...
    component.state.issues.getOrCreate(_componentIssues().IssuesClasses.MultipleEnvs).data = envs.map(e => e.id);
  }
  ...
  async loadOne(id, consumerComponent) {
    ...
    extensionDataList = _extensionData().ExtensionDataList.mergeConfigs([extensionsFromConsumerComponent, extensionDataList]).filterRemovedExtensions(); // temporarily mutate consumer component extensions until we remove all direct access from legacy to extensions data
