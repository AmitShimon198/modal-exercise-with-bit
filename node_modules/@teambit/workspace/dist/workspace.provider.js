...
async function provideWorkspace([pubsub, cli, scope, component, isolator, dependencyResolver, variants, loggerExt, graphql, ui, bundler, aspectLoader, envs], config, [onComponentLoadSlot, onComponentChangeSlot, onComponentAddSlot, onComponentRemoveSlot], harmony) {
  const bitConfig = harmony.config.get('teambit.harmony/bit');
  const consumer = await getConsumer(bitConfig.cwd);
  if (!consumer) return undefined; // TODO: get the 'workspace' name in a better way

  const logger = loggerExt.createLogger(_constants().EXT_NAME);
  const workspace = new (_workspace().Workspace)(pubsub, config, consumer, scope, component, isolator, dependencyResolver, variants, aspectLoader, logger, undefined, harmony, onComponentLoadSlot, onComponentChangeSlot, envs, onComponentAddSlot, onComponentRemoveSlot, graphql);
  ...
  if (!workspace.isLegacy) {
    _componentLoader().default.registerOnComponentLoadSubscriber(async legacyComponent => {
  ...
  _component().default.registerOnComponentConfigLoading(_constants().EXT_NAME, async id => {
    const componentId = await workspace.resolveComponentId(id); // We call here directly workspace.scope.get instead of workspace.get because part of the workspace get is loading consumer component
  ...
  (0, _consumer2().registerDefaultScopeGetter)(async id => {
  ...
  capsuleCmd.commands = [new (_capsule().CapsuleListCmd)(isolator, workspace), new (_capsule().CapsuleCreateCmd)(workspace, isolator)];
  const commands = [new (_install().default)(workspace, logger), new (_ejectConf().default)(workspace), capsuleCmd];
  const watcher = new (_watcher().Watcher)(workspace, pubsub);

  if (workspace && !workspace.consumer.isLegacy) {
    cli.unregister('watch');
    commands.push(new (_watch().WatchCommand)(pubsub, logger, watcher));
    cli.unregister('link');
    commands.push(new (_link().LinkCommand)(workspace, logger));
  }

  commands.push(new (_tagCmd().Tag)());
