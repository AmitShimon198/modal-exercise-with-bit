...
class UpCommand extends cli_1.BaseCommand {
    ...
    async executeUpClassic() {
            ...
            for (const workspace of project.workspaces) {
                for (const target of [suggestUtils.Target.REGULAR, suggestUtils.Target.DEVELOPMENT]) {
                    ...
                    for (const stringifiedIdent of micromatch_1.default(stringifiedIdents, core_1.structUtils.stringifyIdent(pseudoDescriptor))) {
                        ...
                        const request = core_1.structUtils.makeDescriptor(ident, pseudoDescriptor.range);
            ...
            for (const [/*workspace*/ , /*target*/ , existing, { suggestions, rejections }] of allSuggestions) {
                ...
                if (nonNullSuggestions.length === 0) {
                    ...
                    if (!project.configuration.get(`enableNetwork`)) {
                        report.reportError(core_3.MessageName.CANT_SUGGEST_RESOLUTIONS, `${core_1.structUtils.prettyDescriptor(configuration, existing)} can't be resolved to a satisfying range (note: network resolution has been disabled)\n\n${prettyError}`);
                    }
                    else {
                        report.reportError(core_3.MessageName.CANT_SUGGEST_RESOLUTIONS, `${core_1.structUtils.prettyDescriptor(configuration, existing)} can't be resolved to a satisfying range\n\n${prettyError}`);
                    }
                }
                else if (nonNullSuggestions.length > 1 && !interactive) {
                    report.reportError(core_3.MessageName.CANT_SUGGEST_RESOLUTIONS, `${core_1.structUtils.prettyDescriptor(configuration, existing)} has multiple possible upgrade strategies; use -i to disambiguate manually`);
                }
            }
        });
        ...
        for (const [workspace, target, /*existing*/ , { suggestions }] of allSuggestions) {
            ...
            const areAllTheSame = nonNullSuggestions.every(suggestion => core_1.structUtils.areDescriptorsEqual(suggestion.descriptor, firstSuggestedDescriptor));
