...
class InitCommand extends cli_1.BaseCommand {
    constructor() {
        ...
        this.assumeFreshProject = clipanion_1.Option.Boolean(`--assume-fresh-project`, false, { hidden: true });
    }
    async execute() {
    ...
    async executeRegular(configuration) {
        ...
        if (!fslib_1.xfs.existsSync(this.context.cwd))
            await fslib_1.xfs.mkdirPromise(this.context.cwd, { recursive: true });
        const manifest = (await core_1.Manifest.tryFind(this.context.cwd)) || new core_1.Manifest();
        const fields = Object.fromEntries(configuration.get(`initFields`).entries());
        manifest.load(fields);
        manifest.name = (_a = manifest.name) !== null && _a !== void 0 ? _a : core_2.structUtils.makeIdent(configuration.get(`initScope`), fslib_1.ppath.basename(this.context.cwd));
