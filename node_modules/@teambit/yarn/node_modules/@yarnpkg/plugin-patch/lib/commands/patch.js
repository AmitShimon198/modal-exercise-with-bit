...
class PatchCommand extends cli_1.BaseCommand {
    ...
    async execute() {
        ...
        await core_1.StreamReport.start({
            ...
            report.reportInfo(core_1.MessageName.UNNAMED, `Once you are done run ${core_1.formatUtils.pretty(configuration, `yarn patch-commit ${process.platform === `win32` ? `"` : ``}${fslib_1.npath.fromPortablePath(temp)}${process.platform === `win32` ? `"` : ``}`, `cyan`)} and Yarn will store a patchfile based on your changes.`);
        });
    }
}
